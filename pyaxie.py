...
class pyaxie(object):
    ...
    def get_profile_data(self):
		...
		"""
        body = {"operationName": "GetProfileBrief", "variables": {},
    ...
    def download_axie_image(self, axie_id):
        ...
        img_data = requests.get(
            'https://storage.googleapis.com/assets.axieinfinity.com/axies/' + axie_id + '/axie/axie-full-transparent.png').content
        if len(img_data) <= 500:
    ...
    def claim_slp(self):
        ...
        response = requests.post(self.url_api + f"clients/{self.ronin_address}/items/1/claim", headers=custom_headers,
                                 json="")
    ...
    def payout(self):
        ...
        if self.payout_percentage == 0:
            print("Sending all the {} SLP to you : {} ".format(academy_payout_amount,
                                                               self.config['personal']['ronin_address']))
            txns.append(str(self.transfer_slp(self.config['personal']['ronin_address'],
                                              academy_payout_amount + scholar_payout_amount)))
            txns.append("Nothing to send to scholar")
            return txns
        else:
            print("Sending {} SLP to {} : {} ".format(academy_payout_amount, "You",
                                                      self.config['personal']['ronin_address']))
    ...
    def get_prices_from_timestamp(self, timestamp):
        ...
        price_history = cg.get_coin_history_by_id(id='smooth-love-potion', date=dt.date().strftime('%d-%m-%Y'),
                                                  vsCurrencies=['usd'])
        slp_price = price_history['market_data']['current_price']['usd']

        price_history = cg.get_coin_history_by_id(id='axie-infinity', date=dt.date().strftime('%d-%m-%Y'),
                                                  vsCurrencies=['usd'])
        axs_price = price_history['market_data']['current_price']['usd']

        price_history = cg.get_coin_history_by_id(id='ethereum', date=dt.date().strftime('%d-%m-%Y'),
                                                  vsCurrencies=['usd'])
    ...
    def get_axie_total_breed_cost(self, axie_id, txs={}):
        ...
        l = list()
        for i in txs:
            if len(i['logs']) == 4 and len(i['logs'][3]['topics']) > 1 and int(i['logs'][3]['topics'][1],
                                                                               16) in children:
